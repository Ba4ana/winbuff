    ...
    if [[ "$stage" == "1" ]]; then
        ...
        for USER_HOME in /root /home/tech; do
            if [ -d "$USER_HOME" ]; then
                ...
                grep -Fxq "export HISTIGNORE='shutdown:reboot:history*:exit:ls:mc:htop:btop:apt*'" "$USER_HOME/.bash_profile" || echo "export HISTIGNORE='shutdown:reboot:history*:exit:ls:mc:htop:btop:apt*'" >> "$USER_HOME/.bash_profile"
